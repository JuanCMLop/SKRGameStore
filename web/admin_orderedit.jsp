        ...
        <div class="order_box">
            <form action="admin_orderedit.jsp?actiontype=save" method="Post">
                ...
                <table class="order_table">               
                    <tr><td><h5><i>Usuario: </i></h5></td><td>${order.userName}</td><td><input type="submit" class="formbutton" value="Guardar"></td></tr>
                    <tr><td><h5><i>Direccion: </i></h5></td><td><input type='text' name='address' id='address' value='${order.address}' required /></td><td></td></tr>
                    <tr><td><h5><i>N° Tarjeta de Credito: </i></h5></td><td><input type='text' name='creditcard' id='creditcard' value='${order.creditCard}' required /></td><td></td></tr>
                    <tr><td><h5><i>N° de Confirmacion: </i></h5></td><td><c:out value="${order.confirmationNumber}"/></td><td></td></tr>
                    <tr><td><h5><i>Fecha de Entrega: </i></h5></td><td><c:out value="${order.formatDeliveryDate}"/></td><td></td></tr>
                </table>
            </form>
            <table cellspacing='0'>
                <tr><th>No.</th><th>Nombre</th><th>Precio</th><th>Cant</th><th>SubTotal</th><th>Accion</th></tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <form action="admin_orderedit.jsp?actiontype=updatequantity" method="Post">
                                ...
                                <input type="submit" class="formbutton2" value="Actualizar">
                            </form>
                        </td>
                        <td><fmt:setLocale value="en_US"/><fmt:formatNumber value="${orderitem.totalCost}" type="currency"/></td>
                        <td>
                            <span><a href='admin_orderedit.jsp?actiontype=updatequantity&orderid=<c:out value="${order.id}"/>&itemid=<c:out value="${orderitem.orderItemId}"/>&quantity=0' class='button3' onclick = "return confirm('¿Estás seguro de eliminar este producto??')">Eliminar</a></span>
                        </td>
                    </tr>
                ...
                <tr><td colspan="2"><a class='fancybox fancybox.iframe button2' href='admin_orderitemedit.jsp'>Añadir Item</a></td><td></td><td></td><td></td><td></td></tr>
            </table>
        </div>       
    </div>
</section>
