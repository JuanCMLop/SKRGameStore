...
<%
    ...
    if (usertype == null || !usertype.equals(Constants.CONST_TYPE_SALESMAN_LOWER)) {
        errmsg = "No tienes autorización para gestionar pedidos!";
    }
        ...
        <div class="order_box">
            <form action="admin_orderadd.jsp?actiontype=save" method="Post">
                <table class="order_table">               
                    ...
                    <tr><td><h5><i>Dirección: </i></h5></td><td><input type='text' name='address' id='address' value='${order.address}' required /></td><td></td></tr>
                    <tr><td><h5><i>N° Tarjeta de Crédito: </i></h5></td><td><input type='text' name='creditcard' id='creditcard' value='${order.creditCard}' required /></td><td></td></tr>
                    <tr><td><h5><i>N° de Confirmación: </i></h5></td><td><c:out value="${order.confirmationNumber}"/></td><td></td></tr>
                    <tr><td><h5><i>Fecha de entrega: </i></h5></td><td><c:out value="${order.formatDeliveryDate}"/></td><td></td></tr>
                </table>
            </form>
            <table cellspacing='0'>
                <tr><th>No.</th><th>Nombre</th><th>Precio</th><th>Cant</th><th>SubTotal</th><th>Acción</th></tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <span><a href='admin_orderadd.jsp?actiontype=updatequantity&orderid=<c:out value="${order.id}"/>&itemid=<c:out value="${orderitem.orderItemId}"/>&quantity=0' class='button3' onclick = "return confirm('¿Estás seguro de eliminar este producto?')">Eliminar</a></span>
                        </td>
                    </tr>
                ...
                <tr><td colspan="2"><a class='fancybox fancybox.iframe button2' href='admin_orderitemadd.jsp'>Añadir Producto</a></td><td></td><td></td><td></td><td></td></tr>
            </table>
        </div>       
    </div>
</section>
